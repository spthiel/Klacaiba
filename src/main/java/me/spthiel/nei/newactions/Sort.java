...
public class Sort extends ScriptAction implements IDocumentable {
    
    public Sort() {
    
        super(ScriptContext.MAIN, "sort");
    }
    ...
    public IReturnValue execute(IScriptActionProvider provider, IMacro macro, IMacroAction instance, String rawParams, String[] params) {
        ...
        if(params.length > 0) {
            ...
            for(;idx < params.length; idx++) {
                if (Variable.getValidVariableOrArraySpecifier(params[idx]) != null) {
                    String arrayName = Variable.getValidVariableOrArraySpecifier(params[idx]);
