...
public class For extends ScriptActionDo {
	...
	public boolean executeStackPush(IScriptActionProvider provider, IMacro macro, IMacroAction instance, String rawParams, String[] params) {
		...
		try {
			...
			Game.addChatMessage("Something went wrong in the for loop (couldn't get constructor) please report this to Elspeth with the attachment of your log file");
			e.printStackTrace();
			return false;
		}
		...
		if (params.length <= 0) {
			...
			if (state == null) {
				...
				try {
					...
					Game.addChatMessage("Something went wrong in the for loop (" + e.getMessage() + ") please report this to Elspeth with the attachment of your log file");
					e.printStackTrace();
				}
			...
			if(!(isActiveO instanceof Boolean)) {
				Game.addChatMessage("Something went wrong in the for loop (isActive is not a boolean) please report this to Elspeth with the attachment of your log file");
				return false;
			}
			...
			if(!(getVariableName instanceof String)) {
				Game.addChatMessage("Something went wrong in the for loop (getVariableName is not a string) please report this to Elspeth with the attachment of your log file");
				System.out.println("GetVariable: " + getVariableName.getClass().getName());
				return false;
			}

			if(!(getCounter instanceof Integer)) {
				Game.addChatMessage("Something went wrong in the for loop (getCounter is not an integer) please report this to Elspeth with the attachment of your log file");
